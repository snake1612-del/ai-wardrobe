...
import { cancelImport } from "@/modules/import/server/import-capability";
import {
  importErrorResponse,
...
export async function POST(
  ...
  try {
    ...
    if (!sessionId.success || !input.success) {
      throw new ApplicationError("validation", "Отмена импорта недоступна.");
    }
    const version = await cancelImport(accountId, sessionId.data, input.data);
    return NextResponse.json({ version, state: "cancelled" }, { headers: importPrivateHeaders });
  } catch (error) {
