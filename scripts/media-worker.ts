...
const { runMediaWorkerOnce } = await import("../src/modules/media/server/media-worker");

const workerId = `media-worker-${randomUUID()}`;
const drain = process.argv.includes("--drain");
let result: Awaited<ReturnType<typeof runMediaWorkerOnce>>;

do {
  result = await runMediaWorkerOnce(workerId);
  console.info(`Media worker result: ${result}`);
